...
export function CountyMap({ counties, selected, onSelect, metric }) {
  ...
  return <>
    ...
    <div className="map-caption"><span>{active ? `${active.name}, ${active.state} · ${formatValue(active.metrics[metric]?.value)}` : 'Select a colored county'}</span><div className="legend"><span>{formatValue(min)}</span><i/><span>{formatValue(max)}</span><span className="missing-swatch"/> No data</div></div>
  </>;
}
