...
export default function Dashboard() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><a className="brand" href="#overview"><Activity size={36}/><span>PulsePoint</span></a><p className="brand-caption">People. Places. Health.</p>
      <nav aria-label="Main navigation">{[[House,'Overview','overview'],[MapIcon,'Explore counties','explore'],[ChartNoAxesCombined,'Compare','compare'],[Info,'About the data','about']].map(([Icon,label,id])=><a key={id} href={`#${id}`}><Icon size={20}/>{label}</a>)}</nav>
      <div className="sidebar-note"><Activity/><p>Understanding begins<br/>with a closer look.</p></div>
    </aside>
    <main id="overview">
      <header className="topbar"><label className="search"><Search size={20}/><input aria-label="Search counties" placeholder="Search a county or state" value={search} onChange={e=>{setSearch(e.target.value);}}/></label><span className={`status ${sample?'sample':''}`}>{busy?'Loading data':sample?'Sample data':'County estimates'}</span></header>
